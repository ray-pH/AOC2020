...
genRegex rs r
    ...
    | (length $ refs r) == 1 = let rules = map (getRuleById rs) (head $ refs r)
                                in foldr (++) "" $ map (genRegex rs) rules
    | otherwise = let [rules1,rules2] = map (map (getRuleById rs)) $ refs r -- [[Rule]]
                      res1 = foldr (++) "" $ map (genRegex rs) rules1 -- String
                      res2 = foldr (++) "" $ map (genRegex rs) rules2 -- String
                   in "(" ++ res1 ++ "|" ++ res2 ++ ")"
