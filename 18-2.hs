...
evalUnit (x:[]) = [x]
evalUnit (x:"+":y:xs) = evalUnit $ (show evaluated) : xs
        where evaluated = (+) (read x) (read y)
evalUnit (x:"*":y:xs)
    | elem "+" xs = x : "*" : evalUnit (y:xs)
    | otherwise   = evalUnit $ (show evaluated) : xs
    where evaluated = (*) (read x) (read y)

tryEvalUnit :: [String] -> [String]
tryEvalUnit st = if st' == st then st else tryEvalUnit st'
        where st' = evalUnit st
